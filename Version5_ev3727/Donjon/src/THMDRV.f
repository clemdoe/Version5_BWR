...
*     >      DCOOL(K)*VGKprime(K)))/2*(PCOOL(K-1)-PCOOL(K)) 
*     >      + QCOOL(K-1)*HZ(K) +(EPS(K-1)*DGCOOL(K-1)*DLCOOL(K-1)/
*     >      DCOOL(K-1)*HLV(K-1)*VGKprime(K-1))-(EPS(K)*DGCOOL(K)*
*     >      DLCOOL(K)/DCOOL(K)*HLV(K)*VGKprime(K))
        HMSUP = HMSUP * (DCOOL(K)*VCOOL(K)/DCOOL(K-1)*VCOOL(K-1))+DELTA
        WRITE(6,*) 'HMSUP:', HMSUP
       ENDIF
