...
*  ALLOCATABLE ARRAYS
*----
      REAL, ALLOCATABLE, DIMENSION(:) :: VCOOL,TCENT,DLCOOL,DGCOOL, VGJ
...
*----
      ...
      ALLOCATE(DLCOOL(NZ),DGCOOL(NZ),VGJ(NZ))
...
*CGT
     ...
     >    IHCONV,KHCONV,ISUBM,RAD(NDTOT-1,K),ZF,VCOOL(K),PHI2,XFL(K),
     >    EPS(K),SLIP(K),ACOOL,PCH,HZ(K),TCALO,RHO,RHOL,RHOG,
     >    TRE11(NDTOT),KWA(K),VGJ(K))
