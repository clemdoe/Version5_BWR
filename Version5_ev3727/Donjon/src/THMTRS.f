...
*CGT
     ...
     >    IFLUID,IHCONV,KHCONV,ISUBM,RAD(NDTOT-1,K),ZF,VELOT(K),PHI(K),
     >    XFL(K),EPS(K),SLIP(K),ACOOL,PCH,HZ(K),TCALO,DCOOL(K),DLIQT(K), 
     >    DGCOOL(K),TRE11(NDTOT),KWA(K),VGprime,HLV(K))

        ELSEIF (IFLUID.EQ.2) THEN
          CALL THMSAL(IMPX,1,IX,IY,K,K0,MFLXT(K),ENTHT(K),ENT,HD,
