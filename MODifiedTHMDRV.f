*DECK THMDRV
      SUBROUTINE MODTHMDRV(MPTHM,IMPX,IX,IY,NZ,XBURN,VOLXY,HZ,CFLUX,POROS,
...
*----
      ...
      REAL XS(4),TC1,PC(NZ),TP(NZ),RHOL,RHOG,XFL(NZ),EPS(NZ),HINLET,
...
*---
        ...
        IF (IPRES.EQ.1) THEN
          ...
          IF(TCOOL(K).LT.TB) THEN
            IF(IFLUID.EQ.0) THEN
              !CALL THMPT(PCOOL(K),TCOOL(K),RHOIN,HCOOL(K),K11,MUT(K),
              CALL THMPT(PCOOL(K),TCOOL(K),RHOIN,HMSUP,K11,MUT(K),
     > C11)
            ELSE IF(IFLUID.EQ.1) THEN
              !CALL THMHPT(PCOOL(K),TCOOL(K),RHOIN,HCOOL(K),K11,MUT(K),
              CALL THMHPT(PCOOL(K),TCOOL(K),RHOIN,HMSUP,K11,MUT(K),
     > C11)
            ELSE IF(IFLUID.EQ.2) THEN
              !CALL THMSPT(STP,TCOOL(K),R11,HCOOL(K),K11,MUT(K),C11,IMPX)
              CALL THMSPT(STP,TCOOL(K),R11,HMSUP,K11,MUT(K),C11,IMPX)
            ENDIF
          ELSE
            IF(IFLUID.EQ.0) THEN
              !CALL THMPT(PCOOL(K),TB,R11,HCOOL(K),K11,MUT(K),C11)
              CALL THMPT(PCOOL(K),TB,R11,HMSUP,K11,MUT(K),C11)
            ELSE IF(IFLUID.EQ.1) THEN
              !CALL THMHPT(PCOOL(K),TB,R11,HCOOL(K),K11,MUT(K),C11)
              CALL THMHPT(PCOOL(K),TB,R11,HMSUP,K11,MUT(K),C11)
            ELSE IF(IFLUID.EQ.2) THEN
              !CALL THMSPT(STP,TB,R11,HCOOL(K),K11,MUT(K),C11,IMPX)
              CALL THMSPT(STP,TB,R11,HMSUP,K11,MUT(K),C11,IMPX)
            ENDIF
          ENDIF
        ENDIF
      ENDDO
...
*CGT
        IF ((IFLUID.EQ.0).OR.(IFLUID.EQ.1)) THEN
          !CALL THMH2O(0,IX,IY,K,K0,PCOOL(K),MFLOW,HCOOL(K),ENT,HD,
...
*----
      ...
      DEALLOCATE(PTEMP, VTEMP,DGCOOL,VGJprime,HLV)
      DEALLOCATE(A)
      RETURN
*
