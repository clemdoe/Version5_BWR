...
class statesVariables():
    ...
    def getQuality(self, i):
        ...
        if correl == 'simple':
            ...
            H = self.H[i]
            hfg = IAPWS97(P = self.P[i]*(10**(-6)), x = 0).h #Marie: qu'est-ce que hfg ? 
            if H*0.001 < hl:
                x = (H*0.001 - hl)/(hg - hl)
                return 0
            elif H*0.001 > hg:
            ...
            if epriCorrel == "1":
                if xeq >= Xs:
                    ...
                    u = self.U[i]
                    muf = IAPWS97(P = p*(10**(-6)), x = 1).mu #Marie: qu'est-ce que muf ? 
                    Re = rhol * abs(u) * self.D_h[i] / muf #Marie: à quoi correspond ce Reynolds ? 

                    Cpf = IAPWS97(P = p*(10**(-6)), x = 1).cp #Marie: Gloablement, que veut dire l'indice f ? 
                    k_f = IAPWS97(P = p*(10**(-6)), x = 1).k
                    Pr = Cpf * muf / k_f

                
                    #qdp = (p * self.D_h[i])  /(2 * np.pi * self.rf * self.D_h[i])
                    qdp = self.q__[i] * self.DV / (2 * np.pi * self.rw * self.height) #Marie: qu'est-ce que qpd ? 

                    # Calculate heat transfer coefficients
                    hb = np.exp(p / 4.35e6) / (22.7)**2 * 1000.0  # W/(m^2·K)
                    Chn = 0.2 / 4.0 * self.D_h[i] / self.rf #Marie: que signifient les différents indices ? 
                    ...
                    # Calculate characteristic quality xd  ///////////////PROBLEM HERE #Marie: le problème a été résolu ? 
                    delta_h = hg - hl
                    xd = -Cpf / (delta_h) * (
